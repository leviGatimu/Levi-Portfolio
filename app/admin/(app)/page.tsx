...
export default async function DashboardPage() {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {!analytics.available ? (
        <Card>
          <p className="text-[13px] text-[#555]">Analytics are not enabled yet. Run <code className="rounded bg-[#f2f2f4] px-1.5 py-0.5 text-[12px]">backend/migrations/0006_analytics_and_journey.sql</code> in the Supabase SQL editor and visits will start appearing here.</p>
        </Card>
      ) : null}
      ...
      <div className="grid gap-5 lg:grid-cols-3">
        <Card title="Recently edited" subtitle="Projects" icon={<FolderKanban size={18} />} className="lg:col-span-2" actions={<IconLink href="/admin/projects" label="All projects"><ArrowUpRight size={16} /></IconLink>} bodyClassName="p-0 sm:p-0">
          {content.recent.length === 0 ? (
            <div className="p-6 text-center">
              <p className="text-[13px] text-[#555]">No projects yet.</p>
              <Link href="/admin/projects/new" className="mt-3 inline-flex h-10 items-center gap-2 rounded-full bg-[#111] px-4 text-[13px] font-medium text-white">Create the first one</Link>
            </div>
          ) : (
              ...
              {content.recent.map((p) => (
                <li key={p.id} className="flex items-center justify-between gap-4 px-5 py-3.5">
                  <div className="min-w-0">
                    <Link href={`/admin/projects/${p.id}`} className="text-[14px] font-medium text-[#111] hover:underline">{p.name}</Link>
                    <p className="text-[12px] text-[#8a8a8a]">/work/{p.slug} · edited {formatDate(p.updated_at)}</p>
                  </div>
        ...
        <Card title="Content" icon={<Wrench size={18} />} bodyClassName="pt-1">
          ...
          {content.technologies.usage.length > 0 ? (
            <div className="pt-4">
              <p className="mb-3 text-[12px] font-medium text-[#8a8a8a]">Most used in projects</p>
