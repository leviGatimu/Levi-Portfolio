...
function PortraitSlot({ slot, path, alt, label, hint }: { slot: "home" | "about"; path: string | null; alt: string; label: string; hint: string }) {
  ...
  const submit = async (fd: FormData) => {
    setPreparing(true);
    try {
      action(await prepareFormFiles(fd, "file"));
    } finally {
      setPreparing(false);
    }
  };
  return (
    <div className="rounded-2xl border border-slate-200/80 p-4">
      ...
      <div className="mt-3 flex items-start gap-4">
        ...
        <form action={submit} className="flex flex-1 flex-col gap-2">
          ...
          <div className="flex flex-wrap gap-2">
            <Button type="submit" size="sm" variant="primary" disabled={pending || preparing}>{preparing ? "Preparing" : pending ? "Uploading" : "Upload"}</Button>
