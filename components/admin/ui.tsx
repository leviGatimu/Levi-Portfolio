  ...
  return (
    <button
      ...
      className={cn(
        ...
        variant === "primary" && "bg-blue-600 text-white shadow-[0_8px_20px_-10px_rgba(37,99,235,0.6)] hover:bg-blue-700",
        variant === "secondary" && "border border-slate-200 bg-white text-slate-800 hover:border-blue-300 hover:bg-blue-50/40",
...
export function IconButton({ className, label, ...rest }: React.ComponentProps<"button"> & { label: string }) {
  return (
    <button
      ...
      aria-label={label}
      title={label}
      className={cn("flex h-10 w-10 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-700 transition-colors hover:border-blue-300 hover:bg-blue-50 hover:text-blue-700 disabled:opacity-40", className)}
      {...rest}
    />
  );
}
...
export function IconLink({ className, label, href, children, external }: { className?: string; label: string; href: string; children: React.ReactNode; external?: boolean }) {
  const cls = cn("flex h-10 w-10 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-700 transition-colors hover:border-blue-300 hover:bg-blue-50 hover:text-blue-700", className);
...
const inputBase =
  "w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-[14px] text-slate-900 placeholder:text-slate-400 focus:border-blue-500 focus:outline-none focus:ring-4 focus:ring-blue-500/10 disabled:bg-slate-50 disabled:opacity-60";
  ...
  return (
    <div className={cn("flex flex-col gap-1.5", className)}>
      <div className="flex items-baseline justify-between gap-4">
        ...
        {count ? <span className={cn("text-[11px] tabular-nums", count.value > count.max ? "text-red-600" : "text-slate-400")}>{count.value}/{count.max}</span> : null}
      </div>
      {children}
      {help ? <p className="text-[12px] leading-relaxed text-slate-400">{help}</p> : null}
...
export function Card({ title, subtitle, icon, actions, children, className, bodyClassName, id }: CardProps) {
  return (
    <section id={id} className={cn("scroll-mt-28 rounded-[18px] border border-slate-200/80 bg-white", className)}>
      {title ? (
        <header className="flex items-center justify-between gap-4 border-b border-slate-100 px-4 py-3.5 sm:px-5">
          <div className="flex items-center gap-3">
            {icon ? <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-blue-50 text-blue-600">{icon}</span> : null}
            <div>
...
export function Fieldset({ legend, description, children, id, icon, aside }: { legend: string; description?: string; children: React.ReactNode; id?: string; icon?: React.ReactNode; aside?: React.ReactNode }) {
  return (
    <fieldset id={id} className="scroll-mt-28 rounded-[18px] border border-slate-200/80 bg-white">
      <div className="flex items-center justify-between gap-4 border-b border-slate-100 px-4 py-3.5 sm:px-5">
        <div className="flex items-center gap-3">
          {icon ? <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-blue-50 text-blue-600">{icon}</span> : null}
          <div>
            <legend className="text-[15px] font-semibold text-slate-900">{legend}</legend>
  ...
  return (
    <div className="flex items-start justify-between gap-4 border-b border-slate-100 py-3.5 last:border-b-0">
      <div className="flex items-start gap-3">
        <span className="mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-blue-50 text-blue-600">{icon}</span>
        <div className="min-w-0">
          <p className="text-[14px] font-semibold text-slate-900">{label}</p>
          <div className="mt-0.5 text-[13px] text-slate-500">{children}</div>
        </div>
      </div>
      {action}
    </div>
...
export function Tag({ children, className }: { children: React.ReactNode; className?: string }) {
  return <span className={cn("inline-flex items-center gap-1.5 rounded-full bg-slate-100 px-3 py-1.5 text-[12px] font-medium text-slate-700", className)}>{children}</span>;
}
  ...
  return (
    <span
      className={cn(
        ...
        tone === "accent" && "bg-blue-600 text-white",
  ...
  return (
    <div
      ...
      className={cn(
        ...
        tone === "info" && "border-slate-200/80 bg-slate-50 text-slate-700",
    ...
    <nav aria-label="Breadcrumb" className="flex items-center gap-1.5 text-[13px] text-slate-500">
      {items.map((item, i) => (
        <span key={`${item.label}-${i}`} className="flex items-center gap-1.5">
          ...
          {item.href ? <Link href={item.href} className="hover:text-blue-700">{item.label}</Link> : <span className="font-medium text-slate-900">{item.label}</span>}
        </span>
      ))}
    </nav>
  );
  ...
  return (
    <div className="flex flex-wrap items-end justify-between gap-4">
      <div>
        ...
        {description ? <p className="mt-1 max-w-[62ch] text-[13px] text-slate-500">{description}</p> : null}
      </div>
      {action}
    </div>
  );
