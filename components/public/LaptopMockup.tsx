...
type Props = { src: string | null; alt: string; label?: string };

/** The reference's MacBook frame, showing a real project screenshot. */
export default function LaptopMockup({ src, alt, label }: Props) {
  return (
    <motion.div
    ...
    >
      <div className="relative rounded-t-2xl border-[4px] border-slate-700 bg-slate-800 p-2 pb-0 shadow-2xl">
        <div className="relative flex aspect-[16/10] flex-col overflow-hidden rounded-t-lg border border-slate-800 bg-slate-900">
          {src ? (
          ...
          {label ? (
            <span className="absolute left-4 top-4 rounded-full bg-white/90 px-3 py-1 text-[11px] font-bold uppercase tracking-[0.18em] text-slate-900 shadow-md backdrop-blur">{label}</span>
          ) : null}
        </div>
      </div>
